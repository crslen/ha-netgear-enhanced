...
def get_api(
    ...
    #api: Netgear = NetgearEnhanced(password, username, port, ssl, force_login_v2=True, url=host)
    api: Netgear = NetgearEnhanced(password=password, url=host, ssl=False)
...
class NetgearRouter:
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self._api: NetgearEnhanced = None
    ...
    async def async_get_traffic_meter(self) -> dict[str, Any] | None:
        ...
        async with self._api_lock:
            return await self.hass.async_add_executor_job(
                self._api.get_traffic_meter_statistics
            )
    ...
    async def async_get_speed_test(self) -> dict[str, Any] | None:
        ...
        async with self._api_lock:
            return await self.hass.async_add_executor_job(
                self._api.get_speed_test_result
            )
    ...
    async def async_allow_block_device(self, mac: str, allow_block: str) -> None:
        ...
        _LOGGER.info("block %s and %s", mac, allow_block)
        async with self._api_lock:
            await self.hass.async_add_executor_job(
                self._api.set_block_device_by_mac, False, mac, allow_block
            )

    async def async_get_utilization(self) -> dict[str, Any] | None:
        """Get the system information about utilization of the router."""
        async with self._api_lock:
            return await self.hass.async_add_executor_job(self._api.get_info)
